...
function Get-ExistingTrustedSigningCertificate {
  param([string]$Publisher)
  $cert = Get-ChildItem Cert:\CurrentUser\My -CodeSigningCert -ErrorAction SilentlyContinue |
    Where-Object { $_.Subject -eq $Publisher } |
    Sort-Object NotAfter -Descending |
    Select-Object -First 1
  if (-not $cert) {
    Fail "existing signing certificate not found for publisher: $Publisher"
  }
  $trustedRoot = Get-ChildItem Cert:\CurrentUser\Root -ErrorAction SilentlyContinue | Where-Object Thumbprint -eq $cert.Thumbprint
  $trustedPeople = Get-ChildItem Cert:\CurrentUser\TrustedPeople -ErrorAction SilentlyContinue | Where-Object Thumbprint -eq $cert.Thumbprint
  if (-not $trustedRoot -or -not $trustedPeople) {
    Fail "existing signing certificate is not already trusted for CurrentUser: $($cert.Thumbprint)"
  }
  Write-Log "using existing trusted signing certificate without modifying certificate stores: $($cert.Thumbprint)"
  return $cert
}
...
try {
  ...
  if (-not $DryRun) {
    ...
    if ($ExistingTrustedCertificateOnly) {
      $cert = Get-ExistingTrustedSigningCertificate $publisher
    } else {
